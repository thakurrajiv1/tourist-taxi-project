...
import { getCityDistances, upsertCityDistance, deleteCityDistance, UnauthorizedError } from '../../lib/adminApi';
...
export default function AdminDistancesPage() {
  ...
  function startEdit(d) {
    setFromCityId(d.from_city_id);
    setToCityId(d.to_city_id);
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    setFormError('');
    if (!fromCityId || !toCityId || !distanceKm) { setFormError('From city, to city, and distance are required.'); return; }
    if (fromCityId === toCityId) { setFormError('From and to city cannot be the same.'); return; }

    setSubmitting(true);
    try {
      await upsertCityDistance({ from_city_id: fromCityId, to_city_id: toCityId, distance_km: distanceKm, also_reverse: alsoReverse });
      setFromCityId(''); setToCityId(''); setDistanceKm(''); setAlsoReverse(true);
      loadData();
    } catch (err) {
      if (err instanceof UnauthorizedError) { router.replace('/admin/login'); return; }
  ...
  async function handleDelete(d) {
    if (!window.confirm(`Delete the distance for ${d.from_city_name} → ${d.to_city_name}? Custom-location and calculated-fare quotes using this exact pair will fail until it's re-added.`)) return;
    setRowBusyId(d.id);
    try {
      await deleteCityDistance(d.id);
      loadData();
    } catch (err) {
      if (err instanceof UnauthorizedError) { router.replace('/admin/login'); return; }
      setError(err.message);
  ...
  return (
    <AdminLayout title="City Distances">
      <p style={{ color: 'var(--color-text-muted)', fontSize: 14, marginTop: -8, marginBottom: 20 }}>
        The fare calculator needs a distance on file for any route without a fixed price. Click a row's "Edit" to load it into the form and update the value — saving with the same From/To pair overwrites it.
      </p>
        ...
        <form onSubmit={handleSubmit} className="card">
          ...
          <div className="field">
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14 }}>
              <input type="checkbox" checked={alsoReverse} onChange={(e) => setAlsoReverse(e.target.checked)} />
                  ...
                  <tr key={d.id} style={{ borderTop: '1px solid var(--color-border)' }}>
                    ...
                    <td style={td}>
                      <div style={{ display: 'flex', gap: 6 }}>
                        ...
                        >
                          {rowBusyId === d.id ? '…' : 'Delete'}
                        </button>
                      </div>
                    </td>
                  </tr>
                ))}
                {distances.length === 0 && <tr><td style={td} colSpan={4}>No distances saved yet.</td></tr>}
              </tbody>
            </table>
          )}
        </div>
