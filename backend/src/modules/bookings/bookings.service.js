...
/**
 * Cancels a booking. Any status except an already-completed or
 * already-cancelled trip can be cancelled — including one that already
 * has a driver assigned, since plans change and the driver simply needs
 * to be freed up again (their overlap-conflict check in assignment
 * already only counts 'assigned'/'completed' bookings, so a cancelled
 * one automatically stops blocking that driver's calendar).
 */
...
async function assignDriverToBooking(bookingId, driverId) {
  ...
  const driverResult = await pool.query('SELECT * FROM drivers WHERE id = $1 AND is_active = true', [driverId]);
